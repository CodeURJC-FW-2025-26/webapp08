...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (paginationNav) {
    // Ocultar la paginación tradicional
  ...
  // Genera la misma card que en main.mustache
  function renderPlateCard(p) {
    const image = (p.images && p.images.length > 0)
      ? p.images[0]
      : '/images/default-plate.jpg';

    const allergens = (p.allergens && p.allergens.length > 0)
      ? p.allergens.join(", ")
      : "---";
  ...
  async function loadNext() {
    ...
    try {
      ...
      // Añadir las nuevas cards al contenedor
